...
void RTPass::create() {
	...
	std::vector<VkDescriptorSetLayout> descSetLayouts{ AMC::Scene::vkDescSetLayout()->vkDescSetLayout() };
...
void RTPass::execute(const AMC::Scene* scene) {
	...
	vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 0, 1, &scene->descSet, 0, nullptr);
